...
#include <Urho3D/Math/float3x3.h>
#endif
...
#ifdef MATH_OGRE_INTEROP
	float3x3(const Ogre::float3x3 &m) { Set(&m[0][0]); }
	operator Ogre::float3x3() { return Ogre::float3x3(v[0][0], v[0][1], v[0][2], v[1][0], v[1][1], v[1][2], v[2][0], v[2][1], v[2][2]); }
#endif
#ifdef MATH_BULLET_INTEROP
...
#endif
#ifdef MATH_URHO3D_INTEROP
	float3x3(const Urho3D::float3x3 &m) { Set(m.m00_, m.m01_, m.m02_, m.m10_, m.m11_, m.m12_, m.m20_, m.m21_, m.m22_); }
	operator Urho3D::float3x3() { return Urho3D::float3x3(ptr()); }
#endif
};
