...
public:
	...
	static MUST_USE_RESULT float4 FromSphericalCoordinates(const float3 &spherical) {
#pragma warning( push )
...
#ifdef MATH_OGRE_INTEROP
	float4(const Ogre::float4 &other): x(other.x), y(other.y), z(other.z), w(other.w) {}
	float4 &operator =(const Ogre::float4 &other) { x = other.x; y = other.y; z = other.z; w = other.w; return *this; }
	operator Ogre::float4() const { return Ogre::float4(x, y, z, w); }
#endif
#ifdef MATH_QT_INTEROP
...
#endif
#ifdef MATH_URHO3D_INTEROP
	float4(const Urho3D::float4 &other) : x(other.x_), y(other.y_), z(other.z_), w(other.w_) {}
	operator Urho3D::float4() const { return Urho3D::float4(x, y, z, w); }
#endif
