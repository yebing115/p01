...
static const ConstantRemap s_constant_remaps[] = {
  {CONSTANT_BOOL32, D3D_SVC_SCALAR, D3D_SVT_BOOL, 0, 0},
...
void HLSLWriter::WriteUniformDecl(VarDeclNode* node) {
  ...
  if (node->type_decl->type == VAR_TYPE_SAMPLER_2D) {
    ...
    if (node->user_loc) {
      char loc_str[64];
      snprintf(loc_str, sizeof(loc_str), ": register(s%d)", node->loc);
      WriteString(loc_str);
    }
    WriteString(";\n");
...
void HLSLWriter::WriteVarDecl(VarDeclNode* node, bool write_semicolon) {
  ...
  if (node->type_decl->type == VAR_TYPE_SAMPLER_2D || node->type_decl->type == VAR_TYPE_SAMPLER_2D_SHADOW) {
    if (node->user_loc) {
      char loc_str[64];
      snprintf(loc_str, sizeof(loc_str), ": register(t%d)", node->loc);
      WriteString(loc_str);
    }
  }
  if (node->init) {
